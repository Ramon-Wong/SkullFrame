...
void SendEventMessage(  const gchar * msg){
	gchar* js_code = g_strdup_printf("var event = new CustomEvent('%s', { detail: '%s' }); window.dispatchEvent(event);", msg, msg);
	webkit_web_view_evaluate_javascript( webview, js_code, -1, NULL, NULL, NULL, NULL, NULL);
...
void on_load_changed( WebKitWebView *web_view, WebKitLoadEvent load_event, gpointer user_data) {
	...
	if(load_event == WEBKIT_LOAD_STARTED){
		...
		SendEventMessage( "WEBKIT_LOAD_STARTED");
	}
	...
	if(load_event == WEBKIT_LOAD_REDIRECTED){
		...
		SendEventMessage( "WEBKIT_LOAD_REDIRECTED");
	}
	...
	if(load_event == WEBKIT_LOAD_COMMITTED){
		...
		SendEventMessage( "WEBKIT_LOAD_COMMITED");
	}
	...
	if(load_event == WEBKIT_LOAD_FINISHED){
		...
		SendEventMessage( "WEBKIT_LOAD_FINISHES");
	}	
}
